...
describe('inbox-outbox edge cases', () => {
    ...
    describe('readNewInboxMessages with malformed JSONL mixed with valid', () => {
        it('skips malformed lines and returns only valid messages', () => {
            const inbox = join(TEAMS_IO_DIR, 'inbox', 'w1.jsonl');
            const cursorFile = join(TEAMS_IO_DIR, 'inbox', 'w1.offset');
            // Ensure no stale cursor exists
            if (existsSync(cursorFile)) {
                writeFileSync(cursorFile, JSON.stringify({ bytesRead: 0 }), 'utf-8');
            }
            const validMsg = { type: 'message', content: 'valid', timestamp: '2026-01-01T00:00:00Z' };
            const content = [
                'this is not json',
            ...
            // Verify the file exists and has content using readAll first
            const allMsgs = readAllInboxMessages(EDGE_TEAM_IO, 'w1');
            expect(allMsgs).toHaveLength(1);
            expect(allMsgs[0].content).toBe('valid');
            // readNewInboxMessages should also return the valid message from offset 0
            const newMsgs = readNewInboxMessages(EDGE_TEAM_IO, 'w1');
            expect(newMsgs).toHaveLength(1);
            expect(newMsgs[0].content).toBe('valid');
        });
    });
...
describe('team-registration edge cases', () => {
    ...
    describe('shadow registry handles missing workers array gracefully', () => {
        it('DEFECT: crashes when shadow registry JSON exists but has no workers field', () => {
            // BUG FOUND: registerInShadow reads existing shadow file, then does
            // registry.workers.filter(...) without checking if workers exists.
            // When the JSON file has no "workers" key, registry.workers is undefined,
            // causing "Cannot read properties of undefined (reading 'filter')".
            //
            // This could happen in production if:
            // 1. Shadow file is manually edited or written by another tool
            // 2. A partial write leaves the file in an incomplete state
            //
            // RECOMMENDED FIX: In registerInShadow, add a guard:
            //   registry.workers = (registry.workers || []).filter(...)
            const shadowPath = join(REG_DIR, '.omc', 'state', 'team-mcp-workers.json');
            writeFileSync(shadowPath, JSON.stringify({ teamName: REG_TEAM }));
            // Verify this throws due to the missing workers array
            expect(() => registerMcpWorker(REG_TEAM, 'w1', 'codex', 'gpt-5', 'sess1', '/cwd', REG_DIR)).toThrow(/Cannot read properties of undefined/);
        });
    });
